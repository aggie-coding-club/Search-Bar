...
function App() {
  ...
  const [final, setFinal] = useState([{name: "DOE J", gpa: 3.213, students: 91, grades: {as: 31, bs: 23, cs: 17, ds: 8, fs: 3, qs: 12}}])
  ...
  function search(){
    ...
    .then(res => {
        ...
        setName("No results, please try again.")
        setFinal([{name: "DOE J", gpa: 3.213, students: 213, grades: {as: 31, bs: 23, cs: 17, ds: 8, fs: 3, qs: 12}}])
  ...
  function next(){
    ...
    dict[str1] = {gpa: (parseFloat(o)/j).toFixed(3), students: j, grades: {as: a, bs: b, cs: c, ds: d, fs: f, qs: q}}
    data0[str1] = [{name:'a', value: a , fill: "#23e026"}, {name:'b', value: b, fill: "#08d7f4"}, {name:'c', value: c, fill: "#0502fa"}, {name: 'd', value: d, fill: "#b402fa"}, {name:'f', value: f, fill: "#fa02bc"}, {name:'q', value: q, fill: "#fa0223"}]
    setData1(data0)
    var max = value
    max = 0
  ...
  return (
    ...
    <header class="w3-container" >
      <center>
        <input style={{height: '25px', marginTop: '25px'}} placeholder = "Example: CSCE(121)"
          ref = {newTitle}
          onChange={handleChange}
          onKeyPress={handleKeypress}/>
        <button style={{height:'30px'}} onClick = {search} type = "submit ">Search</button>
      </center>
    </header>
